...
class DockerHandler:
    ...
    def get_container_stats(self, container: containers.Container) -> dict:
        ...
        try:
            statistics = {
                ...
                "cpu_usg_persent": self.__calculate_cpu_usage(stats)
            }
    ...
    def restart_containers(self, containers: list[ContainerConfig]) -> None:
        for container in containers:
            self.restart_container(container.name)

    def stop_containers(self, containers: list[ContainerConfig]) -> None:
        for container in containers:
            self.stop_container(container.name)

    def start_containers(self, containers: list[ContainerConfig]) -> None:
        for container in containers:
            self.start_container(container.name)
                    
    def remove_containers(self, containers: list[ContainerConfig]) -> None:
        for container in containers:
            self.stop_container(container.name)
    ...
    def __compose_files(self, files: list[MappedFile]) -> list[Mount]:
    ...
    def __compose_ports(self, ports: list[Port]) -> dict:
    ...
    def create_container(self, container: ContainerConfig, env_vars: dict, files:list[MappedFile] = None) -> None:
        ...
        try:
            ...
            lg.debug(f'Creating container {container.name} with image {container.image}, hostname {container.hostname} and env {env_vars}')
